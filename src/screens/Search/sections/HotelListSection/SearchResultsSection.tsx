...
export const SearchResultsSection = ({ searchResults, onSearchUpdate }: SearchResultsSectionProps): JSX.Element => {
  ...
  const handleViewMore = (hotelId: string) => {
    ...
    if (selectedHotel) {
      ...
      const hotelDataPackage = {
        ...
        selectedFromPage: Math.ceil((searchResults.hotels.findIndex(h => h.id === hotelId) + 1) / 20)
      };
  ...
  const loadMoreHotels = async () => {
    ...
    try {
      ...
      if (data.success && data.hotels && data.hotels.length > 0) {
        ...
        const newHotels = data.hotels.filter((hotel: Hotel) => !existingIds.has(hotel.id));
        ...
        const updatedResults: SearchResults = {
          ...
          hasMorePages: data.hasMorePages !== false && data.hotels.length === 20,
          currentPage: nextPage,
          timestamp: new Date().toISOString()
        };
        
        console.log(`📊 Total hotels after load more: ${combinedHotels.length}`);
        
        // Update localStorage with new results
        localStorage.setItem('hotelSearchResults', JSON.stringify(updatedResults));
      ...
      <div className="container mx-auto px-4">
            ...
            <div className="mt-4 md:mt-0">
              <div className="text-right">
                ...
                <p className="text-sm text-gray-600">
                  {searchResults.availableHotels} available of {searchResults.totalHotels} total
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          {searchResults.hotels.map((hotel) => (
            <Card key={hotel.id} className="rounded-[20px] shadow-shadow-shape hover:shadow-lg transition-shadow duration-200">
              <CardContent className="p-0">
        ...
        {(searchResults.hasMorePages || searchResults.hotels.length < searchResults.totalHotels) && (
                ...
                <>
                  ...
                  {searchResults.totalHotels > searchResults.hotels.length && (
                    <span className="ml-2 text-sm opacity-80">
                      ({searchResults.totalHotels - searchResults.hotels.length} more available)
                    </span>
                  )}
                </>
              )}
        ...
        {!searchResults.hasMorePages && searchResults.hotels.length >= searchResults.totalHotels && searchResults.hotels.length > 5 && (
          <div className="text-center mt-8 py-4">
            <p className="text-gray-600 text-sm">
              🎉 You've seen all {searchResults.hotels.length} available hotels for this search
            </p>
          </div>
        )}
      </div>
