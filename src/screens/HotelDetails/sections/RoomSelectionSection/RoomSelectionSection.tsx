...
export const RoomSelectionSection = ({
  ...
  const processRooms = (hotel: Hotel): ProcessedRoom[] => {
    ...
    if (roomGroups.length === 0) {
      console.log(
        "⚠️ No room_groups found in hotel data, falling back to rates processing"
      );
    ...
    roomGroups.forEach((roomGroup: RoomGroup, index: number) => {
      try {
        ...
        const fullRoomName = beddingType
          ? `${mainName} - ${beddingType}`
          : mainName;

        // Find matching rates for this room group using rg_hash
        const matchingRates = rates.filter(
          (rate: Rate) => rate.rg_hash === rgHash
        );
        console.log(
          `🔗 Found ${matchingRates.length} rates for room group: ${fullRoomName} (${rgHash})`
        );
        ...
        matchingRates.forEach((rate: Rate) => {
          ...
          if (
            rate.payment_options?.payment_types?.length &&
            rate.payment_options.payment_types.length > 0
          ) {
            const paymentType = rate.payment_options.payment_types[0];
            ratePrice = parseFloat(
              paymentType.show_amount || paymentType.amount || "0"
            );
            currency =
        ...
        if (!bestRate) {
          console.log(
            `⚠️ No matching rates found for room group ${rgHash}, using default pricing`
          );
        ...
        else if (lowerName.includes("quadruple") || lowerName.includes("quad"))
          occupancy = "4 guests";
        ...
        if (roomAmenities.length === 0) {
          roomAmenities = hotel.amenities?.slice(0, 3) || [
            "Free WiFi",
            "Air conditioning",
            "Private bathroom",
          ];
        ...
        if (beddingType) {
          ...
          beddingDisplay = beddingType.replace(/\b\w/g, (l: string) =>
            l.toUpperCase()
          );
        ...
        const roomSize =
          (bestRate as Rate | null)?.rooms?.[0]?.size || "Standard size";

        // Get cancellation policy with type assertion
        const cancellationPolicy =
          (bestRate as Rate | null)?.cancellation_policy?.type ||
          "Free cancellation";

        // Get payment type with type assertion
        const paymentType =
          (bestRate as Rate | null)?.payment_options?.payment_types?.[0]
            ?.type || "Pay at hotel";
    ...
    console.log(
      `✅ Successfully processed ${processedRooms.length} rooms from ${roomGroups.length} room groups`
    );

    // If no rooms were processed, fallback to rates processing
    if (processedRooms.length === 0) {
      console.log(
        "⚠️ No rooms processed from room_groups, falling back to rates"
      );
      return processRoomsFromRates(hotel, rates);
    }
  ...
  const handleQuantityChange = (roomId: string, quantity: number) => {
    console.log(roomId, quantity, "SOme test");
  ...
  const getAmenityIcon = (amenity: string) => {
    ...
    if (lowerAmenity.includes("wifi") || lowerAmenity.includes("internet")) {
      ...
      lowerAmenity.includes("bath") ||
      lowerAmenity.includes("shower")
    ) {
      ...
      <CardContent className="p-6 lg:p-8">
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            Select from {rooms.length} available room type
            {rooms.length !== 1 ? "s" : ""} for your {duration}-night stay
            {hasMoreRooms && (
        ...
        <div className="space-y-6">
          {roomsToDisplay.map((room) => {
            const currentQuantity =
              quantities[room.id] || selectedQuantity || 1;
                      ...
                      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start mb-4">
                          ...
                          <div className="text-xs text-gray-500 space-y-1">
                            ...
                            {room.availability <= 3 && (
                              <div className="text-orange-600 font-medium">
                                ⚠️ Only {room.availability} room
                                {room.availability !== 1 ? "s" : ""} left!
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="lg:text-right lg:min-w-[180px] mt-4 lg:mt-0">
                          ...
                          <div className="mb-4">
                            ...
                            >
                              {[...Array(Math.min(9, room.availability))].map(
                                (_, i) => (
                          ...
                          <Button
                            onClick={() =>
                              handleQuantityChange(room.id, currentQuantity)
                            }
                            className={`w-full ${
                              isSelected
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-app-accent">
                Room Selection Summary
              </h3>
              <p className="text-sm text-gray-600">
                {rooms.length} room type{rooms.length !== 1 ? "s" : ""}{" "}
                available for {duration} night{duration !== 1 ? "s" : ""}
                {hasMoreRooms && (
          ...
          <div className="mt-6 p-4 bg-gray-100 rounded-lg text-xs text-gray-600">
            <details>
              <summary className="cursor-pointer font-medium">
                Enhanced Room Processing Debug Info
              </summary>
              <div className="mt-2 space-y-2">
                <div>
                  <strong>Room Groups Found:</strong>{" "}
                  {hotel.ratehawk_data?.room_groups?.length || 0}
                </div>
                <div>
                  <strong>Rate Entries Found:</strong>{" "}
                  {hotel.ratehawk_data?.rates?.length || 0}
                </div>
                <div>
                  <strong>Rooms Successfully Processed:</strong> {rooms.length}
                </div>
                <div>
                  <strong>Selected Room ID:</strong> {selectedRoomId || "None"}
                </div>
                <div>
                ...
                <details className="mt-2">
                  <summary className="cursor-pointer text-blue-600">
                    Room Groups Breakdown (
                    {hotel.ratehawk_data?.room_groups?.length || 0} total)
                  </summary>
                    ...
                    {hotel.ratehawk_data?.room_groups
                      ...
                      .map((rg: any, i: number) => (
                        <div key={i} className="py-1 border-b border-gray-200">
                          <strong>{rg.rg_hash}:</strong>{" "}
                          {rg.name_struct?.main_name}
                          {rg.name_struct?.bedding_type &&
                            ` - ${rg.name_struct.bedding_type}`}
                        </div>
                      ))}
                ...
                <details className="mt-2">
                  <summary className="cursor-pointer text-green-600">
                    Rate Entries Breakdown (
                    {hotel.ratehawk_data?.rates?.length || 0} total)
                  </summary>
                  <div className="mt-1 max-h-32 overflow-y-auto bg-white p-2 rounded text-xs">
                    {hotel.ratehawk_data?.rates?.map((rate: any, i: number) => (
                      <div key={i} className="py-1 border-b border-gray-200">
                        <strong>Rate {i + 1}:</strong> rg_hash:{" "}
                        {rate.rg_hash || "No hash"} | Price:{" "}
                ...
                <details className="mt-2">
                  <summary className="cursor-pointer text-purple-600">
                    Room Group to Rate Matching Analysis
                  </summary>
                  <div className="mt-1 max-h-40 overflow-y-auto bg-white p-2 rounded text-xs">
                    {hotel.ratehawk_data?.room_groups
                      ...
                      .map((rg: any, i: number) => {
                        const matchingRates =
                          hotel.ratehawk_data?.rates?.filter(
                            (rate: any) => rate.rg_hash === rg.rg_hash
                          ) || [];
                        return (
                          <div
                            key={i}
                          ...
                          >
                            <strong>{rg.rg_hash}:</strong>{" "}
                            {rg.name_struct?.main_name}→ {matchingRates.length}{" "}
                            matching rate{matchingRates.length !== 1 ? "s" : ""}
                            {matchingRates.length > 0 && (
                              <span className="text-blue-600">
                                {" "}
                                ($
                                {matchingRates[0].payment_options
                    ...
                    {(hotel.ratehawk_data?.room_groups?.length || 0) > 10 && (
                      <div className="py-1 text-gray-500">
                        ... showing first 10 of{" "}
                        {hotel.ratehawk_data?.room_groups?.length} room groups
                      </div>
                    )}
                  </div>
                </details>
                ...
                <details className="mt-2">
                  <summary className="cursor-pointer text-orange-600">
                    Successfully Processed Rooms ({rooms.length})
                  </summary>
                  <div className="mt-1 max-h-32 overflow-y-auto bg-white p-2 rounded text-xs">
                    {rooms.map((room, i) => (
                      <div key={i} className="py-1 border-b border-gray-200">
                        <strong>#{i + 1}:</strong> {room.name} | ${room.price}{" "}
                        {room.currency} |{room.occupancy} | {room.bedding} | ID:{" "}
                        {room.id}
                        {room.rgHash && (
                          <span className="text-blue-600">
                            {" "}
                ...
                <details className="mt-2">
                  ...
                  <div className="mt-1 bg-white p-2 rounded text-xs">
                    <div>
                      <strong>Destination:</strong> {searchContext.destination}{" "}
                      (ID: {searchContext.destinationId})
                    </div>
                    <div>
                      <strong>Dates:</strong> {searchContext.checkin} →{" "}
                      {searchContext.checkout} ({duration} nights)
                    </div>
                    <div>
                      <strong>Guests:</strong> {searchContext.guests}
                    </div>
                    <div>
                      <strong>Total Hotels Available:</strong>{" "}
                      {searchContext.totalHotels}
                    </div>
                    <div>
                      <strong>Search Timestamp:</strong>{" "}
                      {searchContext.searchTimestamp}
                    </div>
                  </div>
                </details>
                ...
                <div className="mt-2 p-2 bg-blue-50 rounded">
                  ...
                  <div>
                    Room Processing Success Rate:{" "}
                    {hotel.ratehawk_data?.room_groups?.length
                      ? Math.round(
                          (rooms.length /
                            hotel.ratehawk_data.room_groups.length) *
                            100
